...
Taxi_FareEnd(playerid)
{
	...
	if(!IsPlayerInGroup(playerid, FRAC_KT))
	{
		...
		Log(payLog, WARNING, "%s zarobi³ $%d na s³u¿bie taksówkarza.", GetPlayerLogName(playerid), TransportMoney[playerid]);
	}
    else
    {
        Sejf_Add(FRAC_KT, floatround(TransportMoney[playerid]/2));
		DajKaseDone(playerid, floatround(TransportMoney[playerid]/2));
		Log(payLog, WARNING, "%s zarobil $%d dla KT na s³u¿bie taksówkarza.", GetPlayerLogName(playerid), TransportMoney[playerid]);
    }
...
GetPlayerFrac(playerid) // Duplikat funkcji ¿eby mi siê nie pierdoli³o z legacy funkcjami
{
...
public KradziezAut() //nowy system
{
	foreach(new i : Player)
	{
		...
		switch(KradziezEtap[i])
		{
			case 1:
			{
				format(komunikat, sizeof(komunikat), "* %s ³¹czy kabelki i wyjmuje œrubokrêt i odkrêca nastêpn¹ os³onkê.", GetNick(i));
			...
			case 3:
			{
				...
				if(kradnij <= mnoznik)
				{
					...
					if(PlayerInfo[i][pJackSkill] == 50)
					{ SendClientMessage(i, COLOR_YELLOW, "* Twój skill z³odzieja samochodów wynosi teraz 2, masz teraz wiêksze szanse ¿e uda ci siê ukraœæ wóz."); }
					else if(PlayerInfo[i][pJackSkill] == 100)
					{ SendClientMessage(i, COLOR_YELLOW, "* Twój skill z³odzieja samochodów wynosi teraz 3, masz teraz wiêksze szanse ¿e uda ci siê ukraœæ wóz."); }
					else if(PlayerInfo[i][pJackSkill] == 200)
					{ SendClientMessage(i, COLOR_YELLOW, "* Twój skill z³odzieja samochodów wynosi teraz 4, masz teraz wiêksze szanse ¿e uda ci siê ukraœæ wóz."); }
					else if(PlayerInfo[i][pJackSkill] == 400)
					{ SendClientMessage(i, COLOR_YELLOW, "* Twój skill z³odzieja samochodów wynosi teraz 5, masz teraz wiêksze szanse ¿e uda ci siê ukraœæ wóz."); }
				}
				else
				{
					new vehi = GetPlayerVehicleID(i);
					new engine, lights, alarm, doors, bonnet, boot, objective;
					format(komunikat, sizeof(komunikat), "* %s Ÿle ³¹czy kabelki po czym w³¹cza siê alarm.", GetNick(i));
					ProxDetector(20.0, i, komunikat, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
					...
					SendClientMessage(i, COLOR_PANICRED, "Zaraz tu bêd¹ gliny! Uciekaj!");
					TogglePlayerControllable(i, 1);
...
Kostka_Wygrana(playerid, loser, kasa, bool:quit=false)
{
    ...
    Log(payLog, WARNING, "%s wygra³ rzuty kostk¹ z %s na kwotê %d$ %s", GetPlayerLogName(playerid), GetPlayerLogName(loser), kasa, quit ? "(quit)" : "");
    Sejf_Add(11, podatek);
...
ShowStats(playerid,targetid)
{
    if(IsPlayerConnected(playerid) && IsPlayerConnected(targetid))
	{
		...
		format(coordsstring, sizeof(coordsstring), "Zabiæ:[%d] œmierci:[%d] Bonus Levelowy:[$%d] Respekt:[%d/%d] WL:[%d] Skin ID:[%d] KotnikCoins:[%d]",kills,deaths,costlevel,exp,expamount,wanted, skin, PremiumInfo[targetid][pMC]);
		SendClientMessage(playerid, COLOR_GRAD4,coordsstring);
		format(coordsstring, sizeof(coordsstring), "Drugs:[%d] Mats:[%d] Warny:[%d] Dostêpnych zmian nicków:[%d] Si³a:[%d] G³ód:[%.1f%%]",drugs,mats,PlayerInfo[targetid][pWarns],znick, PlayerInfo[targetid][pStrong], PlayerInfo[targetid][pHunger]);
...
PokazStatyDialogBronie(playerid)
{
	...
	return ShowPlayerDialogEx(playerid, DIALOG_STATS_UMIEJETNOSCI, DIALOG_STYLE_LIST, "Umiejêtnoœci postaci", string, "Zamknij", "");
}
...
PokazStatyDialogSkille(playerid)
{
	...
	return ShowPlayerDialogEx(playerid, DIALOG_STATS_UMIEJETNOSCI, DIALOG_STYLE_LIST, "Umiejêtnoœci postaci", string3, "Zamknij", "");
}
...
PokazStatyDialog(playerid)
{
	...
	format(string, sizeof(string), "%s\n{d9d9d9}---", string);
	format(string, sizeof(string), "%s\n\t{d9d9d9}Wyœwietl posiadane dokumenty", string);
	format(string, sizeof(string), "%s\n\t{d9d9d9}Wyœwietl posiadane tatua¿e", string);
	format(string, sizeof(string), "%s\n\t{d9d9d9}Wyœwietl statystyki postaci", string);
	format(string, sizeof(string), "%s\n\t{d9d9d9}Wyœwietl umiejêtnoœci postaci", string);
	format(string, sizeof(string), "%s\n\t{d9d9d9}Wyœwietl umiejêtnoœci strzelania", string);
...
Dom_ChangeOwner(playerid, dom, forid)
{
	...
	Log(adminLog, WARNING, "Admin %s zmieni³ w³aœciciela domu %d z %s na %s", GetPlayerLogName(playerid), dom, Dom[dom][hWlasciciel], GetPlayerLogName(forid));
...
KupowanieDomu(playerid, dom, platnosc, intType = 0)
{
	...
	else
	{
		...
		if(intType == 1)
		{
			SendClientMessage(playerid, COLOR_NEWS, "INFO: Wybra³eœ w³asny interior. Mo¿esz go teraz zbudowaæ lub wynaj¹æ kogoœ do pomocy.");
			SendClientMessage(playerid, COLOR_NEWS, "INFO: Pod komend¹ /dom mo¿esz ustawiæ pozycjê wejœcia do domu po zbudowaniu interioru.");
		}
...
ZlomowanieDomu(playerid, dom)
{
	...
	if(dini_Exists(plik))
	{
		...
		else
		{
			...
			{
				Log(serverLog, WARNING, "Dom %s zosta³ zez³omowany z powodu nieaktywnoœci w³aœciciela %s.", GetHouseLogName(dom), Dom[dom][hWlasciciel]);
...
NaprawSpojnoscWlascicielaDomu(playerid)
{
	...
	else if(domcheck == 0)
	{
		if(Dom[dom][hKupiony] == 0)
		{
			...
			Log(payLog, WARNING, "%s straci³ dom %s z powodu nieaktywnoœci i otrzyma³ %d$", GetPlayerLogName(playerid), GetHouseLogName(dom), Dom[dom][hCena]/2);
			PlayerInfo[playerid][pDom] = 0;
		}
		else
		{
			...
			Log(payLog, WARNING, "%s straci³ dom %s z powodu nieaktywnoœci (2 w³aœcicieli) i otrzyma³ %d$", GetPlayerLogName(playerid), GetHouseLogName(dom), Dom[dom][hCena]/2);
			PlayerInfo[playerid][pDom] = 0;
		}
	}
	else
...
public OPCLogin(playerid)
{
	...
	if(result == -1 || result == 1) //logowanie
	{
        //Logowanie
		new string[512];
		SendClientMessage(playerid, COLOR_YELLOW, "Witaj na czwartej edycji Kotnik Role Play, zaloguj siê aby poczuæ VIBE.");
...
GPSMode(playerid, bool:red = false)
{
	...
	if(red)
	{
		format(string, sizeof(string), "=: %s %s GPS %s :=", sendername, (PDGPS == playerid) ? ("deaktywowa³") : ("aktywowa³"), (PDGPS == playerid) ? ("- odwo³ujê RED") : ("potrzebne wsparcie! - CODE RED"));
...
Oil_Destroy(lID)
{
    ...
    foreach(new i : Player)
    {
        ...
        SendClientMessage(i, COLOR_WHITE, "[ERS] Usun¹³eœ plamê oleju! Otrzymujesz "#PRICE_PLAMA"$! [ERS]");
        DajKaseDone(i, PRICE_PLAMA);
        GroupSendMessage(4, COLOR_GREEN, "[ERS] Stra¿ak usun¹³ plamê oleju! Na konto frakcji wp³ywa "#PRICE_PLAMA"$! [ERS]");
		SendMessageToAdmin(sprintf("PLAMY: Gracz %s [%d] usun¹³ plamê oleju i otrzyma³ za to "#PRICE_PLAMA"$!", GetNickEx(i), i), COLOR_LIGHTRED);
		Log(payLog, WARNING, "%s usun¹³ plamê oleju i otrzyma³ za to "#PRICE_PLAMA"$", GetPlayerLogName(i));
            ...
            else
            {
				if(IsPlayerInRangeOfPoint(playerid, 20, TransportJobData[idx2][eTJDEndX], TransportJobData[idx2][eTJDEndY], TransportJobData[idx2][eTJDEndZ]))
				{
					if(GetPVarInt(playerid, "trans-antytp") > gettime())
					{
						...
						Log(punishmentLog, WARNING, "%s dosta³ kicka od antycheata, powód: teleport (kurier)", GetPlayerLogName(playerid));
			        	KickEx(playerid);
					}
					idx = (idx-1)/2;
...
ShowPersonalization(playerid, value)
{
	...
	if(value == 2)
	{
		...
		ShowPlayerDialogEx(playerid, D_PERS_CHAT, DIALOG_STYLE_TABLIST, "Kotnik RP", string, "Akceptuj", "Odrzuæ"); 
...
PursuitMode(playerid, giveplayerid)
{
	if(ProxDetectorS(80.0, playerid, giveplayerid))
	{
		...
		{
			SendClientMessage(playerid,COLOR_LIGHTBLUE,"Rozpocz¹³eœ poœcig! Trwa on 10 minut.");
...
public DeathAdminWarning(playerid, killerid, reason)
{
	...
	if(killerid != INVALID_PLAYER_ID)
	{
		...
		{
			//-------<[  Logi  ]>---------
			Log(warningLog, WARNING, "%s %s %s z broni o id %d bêd¹c w aucie (mo¿liwe DB/CK2).", GetPlayerLogName(killerid), bwreason, GetPlayerLogName(playerid), reason);
					...
					if(GetPVarInt(playerid, "skip_bw")  == 0)
					{
						if(PlayerInfo[playerid][pInjury] > 0)
						{
							if(lowcaz[killerid] == playerid && lowcap[playerid] != killerid && poddaje[playerid] != 1)
							{
								...
								SendMessageToAdmin(string, COLOR_YELLOW);
								Log(warningLog, WARNING, "³owca nagród %s %s gracza %s bez oferty /poddajsie", GetPlayerLogName(killerid), bwreason, GetPlayerLogName(playerid));
							}
...
PrintDutyTime(playerid, reset = 1)
{
	...
	godziny = floatround( sekundy / 3600 );
	minuty = floatround( sekundy / 60 % 60 );
	format(string, sizeof(string), "[DUTY] Spêdzi³eœ na s³u¿bie %d minut %d godzin.", minuty, godziny);
	SendClientMessage(playerid, COLOR_GREEN, string);
	if(reset)
	{
		sendTipMessage(playerid, "Twój czas na s³u¿bie zostanie wyzerowany za 15 minut.");
		SetPVarInt(playerid, "reset-dutytime", gettime()+900);
		if(OnDuty[playerid] > 0)
		{
			new orgid = GetPlayerGroupUID(playerid, OnDuty[playerid]-1);
			Log(serverLog, WARNING, "%s spêdzi³ na s³u¿bie %d minut %d godzin [%s]", GetPlayerLogName(playerid), minuty, godziny, GetGroupLogName(orgid));
		}
	}
	return 1;
}
...
ShowNewRadio(playerid)
{
	...
	ShowPlayerDialogEx(playerid, D_NEWRADIO, DIALOG_STYLE_LIST, "Radio", string, "Wybierz", "WyjdŸ");
}
...
UpdateRybyTop()
{
    ...
    mysql_store_result();
	format(RybyTopString, sizeof(RybyTopString), "{279127}Najwiêksze ryby:\n");
