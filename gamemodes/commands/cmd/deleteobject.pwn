            ...
            for(new i=0;i<25000;i++)
            {
                if(IsValidDynamicObject(i))
                {
                    if(Streamer_GetIntData(STREAMER_TYPE_OBJECT, i, E_STREAMER_WORLD_ID) == data)
                    {
                        ...
                        Log(adminLog, WARNING, "Admin %s usun¹³ obiekt %d - model %d at [%.1f, %.1f, %.1f]", GetPlayerLogName(playerid), i, Streamer_GetIntData(STREAMER_TYPE_OBJECT, i, E_STREAMER_MODEL_ID), x, y, z);
            ...
            for(new i=0;i<25000;i++)
            {
                if(IsValidDynamicObject(i))
                {
                    if(Streamer_GetIntData(STREAMER_TYPE_OBJECT, i, E_STREAMER_WORLD_ID) == data)
                    {
                        ...
                        {
                            Log(adminLog, WARNING, "Admin %s usun¹³ VW.%d dist.%d object %d - model %d at [%.1f, %.1f, %.1f]", GetPlayerLogName(playerid), data, extra, i, Streamer_GetIntData(STREAMER_TYPE_OBJECT, i, E_STREAMER_MODEL_ID), x, y, z);
                            DestroyDynamicObject(i);
                            count++;
