...
YCMD:scena(playerid, params[], help)
{
    if(CheckPlayerPerm(playerid, PERM_NEWS) && PlayerInfo[playerid][pAdmin] < 200)
    {
        ...
        {
            if(GroupPlayerDutyRank(playerid) < 4) return sendTipMessageEx(playerid, COLOR_GRAD2, "Scena dostêpna za pozwoleniem adminów i od 4 rangi!");
            else
            {
    ...
    {
        ShowPlayerDialogEx(playerid, SCENA_DIALOG_GETMONEY, DIALOG_STYLE_MSGBOX, "Kotnik Role Play", "Aby postawiæ scenê w swoim zakresie musisz umieœciæ op³atê\nW sejf San News w wysokoœci 2 milionów!!!!\nJe¿eli organizujesz event OOC zg³oœ siê do lidera SN po pozwolenie na scenê (darmow¹)", "Zap³aæ", "Anuluj"); 
        return 1;
    }
