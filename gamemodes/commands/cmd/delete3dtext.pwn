            ...
            for(new i=0;i<20000;i++)
            {
                if(IsValidDynamic3DTextLabel(Text3D:i))
                {
                    if(Streamer_GetIntData(STREAMER_TYPE_3D_TEXT_LABEL, Text3D:i, E_STREAMER_WORLD_ID) == _:data)
                    {
                        ...
                        Log(adminLog, WARNING, "Admin %s usun¹³ 3DText %d - at [%.1f, %.1f, %.1f]", GetPlayerLogName(playerid), i, x, y, z);
            ...
            for(new i=0;i<20000;i++)
            {
                if(IsValidDynamic3DTextLabel(Text3D:i))
                {
                    if(Streamer_GetIntData(STREAMER_TYPE_3D_TEXT_LABEL, Text3D:i, E_STREAMER_WORLD_ID) == _:data)
                    {
                        ...
                        {
                            Log(adminLog, WARNING, "Admin %s usun¹³ VW VW.%d dist.%d Text3D %d - at [%.1f, %.1f, %.1f]",  GetPlayerLogName(playerid), _:data, extra, i, x, y, z);
                            DestroyDynamic3DTextLabel(Text3D:i);
                            count++;
