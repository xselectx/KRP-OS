    ...
    if(IsPlayerConnected(playerid))
   	{
		if((IsAPolicja(playerid) || IsAFBI(playerid)) && GroupPlayerDutyRank(playerid) >= 3)
		{
			...
			if(time < 1 || time > 60) { sendTipMessageEx(playerid, COLOR_GREY, "Czas wiêzienia od 1 do 60 minut!"); return 1; }
			...
			if(IsPlayerConnected(suspect))
			{
				if(GetDistanceBetweenPlayers(playerid,suspect) < 5)
				{
					...
					if(IsAFBI(playerid)){
						...
						format(string, sizeof(string), "Uwiêzi³eœ %s, nagroda za przestêpcê: %d. Otrzymujesz $%d", giveplayer, moneys, depo3+100);
