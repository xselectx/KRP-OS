    ...
    if(CheckPlayerPerm(playerid, PERM_TAXI))
    {
    	...
    	else if(strcmp(data,"koszt",true) == 0)
    	{
    	    if(GroupPlayerDutyRank(playerid) >= 4)
    	    {
    		    ...
    		    if(koszt < PRICE_LOT_MIN || koszt > PRICE_LOT_MAX) { sendTipMessageEx(playerid, COLOR_GREY, "Cena biletu nie mo¿e byæ mniejsza od "#PRICE_LOT_MIN"$ i nie wiêksza od "#PRICE_LOT_MAX"$"); return 1; }
                kosztbiletu = koszt;
                if(kosztbiletu>0)
    		    {
        			GetPlayerName(playerid, sendername, sizeof(sendername));
