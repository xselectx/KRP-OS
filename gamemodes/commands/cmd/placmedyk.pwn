	    ...
	    if(giveplayerid != INVALID_PLAYER_ID)
	    {
	        if(ProxDetectorS(8.0, playerid, giveplayerid) && IsPlayerInGroup(giveplayerid, 4))
			{
			    ...
			    if(kaska[playerid] >= money)
		        {
			        ...
			        Log(payLog, WARNING, "%s da³ medykowi %s kwotê %d$, która trafi³a do sejfu LSMC (stan: %d$)", GetPlayerLogName(playerid), GetPlayerLogName(giveplayerid), money, GroupInfo[4][g_Money]);
			        return 1;
				}
				else
				{
