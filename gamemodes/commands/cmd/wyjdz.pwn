...
YCMD:wyjdz(playerid, params[], help)
{
    if(IsPlayerConnected(playerid))
	{
		...
		else if(IsPlayerInRangeOfPoint(playerid, 2, 1994.08, -2127.61, 30.56) && GetPlayerVirtualWorld(playerid) == 25 && PlayerInfo[playerid][pJailed] == 0) // Wyjœcie z celi
		{
        ...
        else if (PlayerToPoint(2.0, playerid,211.7162,1811.4824,21.8594) && (PlayerInfo[playerid][pJailed] == 0)) //Wejœcie do wiêzienia stanowego
        {
            ...
            {
                SetPlayerPos(playerid,-1606.0139,818.9503,-29.4141); //Wejœcie do wiêzienia stanowego
		...
		else
		{
			...
			for(new i; i<=MAX_NrDOM; i++)
		    {
				if(IsPlayerInRangeOfPoint(playerid, 5.0, IntInfo[i][Int_X], IntInfo[i][Int_Y], IntInfo[i][Int_Z]))
				{
					...
					if(Dom[deem][hVW] == GetPlayerVirtualWorld(playerid))
					{
		    ...
		    {
				if(IsPlayerInRangeOfPoint(playerid, 5.0, Dom[i][hInt_X], Dom[i][hInt_Y], Dom[i][hInt_Z]))
				{
                    new deem = i;
					if(Dom[deem][hVW] == GetPlayerVirtualWorld(playerid))
					{
