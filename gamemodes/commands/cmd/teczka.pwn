...
YCMD:teczka(playerid, params[], help)
{
    ...
    if(IsPlayerConnected(playerid))
    {
        if(gPlayerLogged[playerid] == 1)
        {
			...
			{
				sendTipMessage(playerid, "Zanim bêdziesz móg³ p³aciæ, musisz graæ wiêcej ni¿ 2 godzinê online!");
			    return 1;
			}

			if(moneys < 1 || moneys > 10001)
			{
			    sendTipMessage(playerid, "Kwota musi wynosiæ mniej ni¿ 10000$ .");
			    return 1;
			}
			    ...
			    if(giveplayerid != INVALID_PLAYER_ID && playerid != giveplayerid)
			    {
					...
					if (ProxDetectorS(5.0, playerid, giveplayerid) && Spectate[giveplayerid] == INVALID_PLAYER_ID)
					{
						...
						if (moneys > 0 && playermoney >= moneys)
						{
							...
							Log(payLog, WARNING, "%s da³ %s kwotê %d$", GetPlayerLogName(playerid), GetPlayerLogName(giveplayerid), moneys);
