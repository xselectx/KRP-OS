...
YCMD:checkcar(playerid, params[], help)
{
	if (PlayerInfo[playerid][pAdmin] >= 1 || PlayerInfo[playerid][pNewAP] >= 1 || Uprawnienia(playerid, ACCESS_PANEL)  || PlayerInfo[playerid][pAdmin] == 7 || IsAScripter(playerid))
	{
        ...
        new str[128];
        format(str, 128, "System: Pojazd nale¿y do %s - %s (UID: %d)", (CarData[uid][c_OwnerType] >= 6) ? ("ZBUGOWANY W£AŒCICIEL") : (CarOwnerNames[CarData[uid][c_OwnerType]]), Car_PrintOwner(uid), CarData[uid][c_UID]);
