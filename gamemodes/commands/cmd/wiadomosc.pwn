    ...
    if(IsPlayerConnected(giveplayerid) && giveplayerid != INVALID_PLAYER_ID)
    {
        ...
        if(Kajdanki_JestemSkuty[playerid] == 1  && !PlayerInfo[playerid][pAdmin] && !PlayerInfo[playerid][pNewAP] && !PlayerInfo[playerid][pZG] && !IsAScripter(playerid))
        {
            if(GetDistanceBetweenPlayers(playerid, giveplayerid) > 50.0)
            {
                if(!PlayerInfo[giveplayerid][pAdmin] && !PlayerInfo[giveplayerid][pNewAP] && !PlayerInfo[giveplayerid][pZG] && !IsAScripter(giveplayerid)) 
                {
                    return sendErrorMessage(playerid, "Gdy jesteœ skuty mo¿esz wysy³aæ wiadomoœci jedynie na ma³¹ odleg³oœæ.");
                }
            }
        }
        ...
        if (AntyCzitText(text))
        {
            ...
            Log(warningLog, WARNING, "%s mówi coœ o czitach na PW do %s: %s", GetPlayerLogName(playerid), GetPlayerLogName(giveplayerid), text);
        }
        ...
        if(GetPlayerAdminDutyStatus(giveplayerid) == 1 && PlayerInfo[playerid][pAdmin] == 0)
		{
			...
			format(string, sizeof(string), "Pisz¹c wiadomoœæ do Administracji b¹dŸ GameMastera masz obowi¹zek przepisania Tokena.\nTwój token to: %d",Token[playerid]);
			ShowPlayerDialogEx(playerid,7981,DIALOG_STYLE_INPUT,"Wiadomoœæ do administracji - token",string,"Wybierz","Anuluj");
			return 1;
		}
        if(strlen(params) < 78)
        {
