...
YCMD:caluj(playerid, params[], help)
{
    ...
    if(IsPlayerConnected(playerid))
    {
		...
		{
			sendErrorMessage(playerid, "Nie mo¿esz ca³oœæ siê w trakcie obecnego statusu (ranny)!"); 
			return 1;
		}
		if(playa == playerid)
		{
		    ...
		    if(IsPlayerConnected(playa))
		    {
		        if(playa != INVALID_PLAYER_ID)
		        {
					...
					format(string, sizeof(string), "%s chce siê z tob¹ poca³owaæ - jeœli go kochasz kliknij ''Ca³uj''!", GetNick(playerid));
  					ShowPlayerDialogEx(playa, 1092, DIALOG_STYLE_MSGBOX, "Kotnik Role Play - poca³unek", string, "Ca³uj", "Odrzuæ", false);
					ShowPlayerInfoDialog(playerid, "Kotnik Role-Play", "Zaoferowa³eœ poca³unek - oczekuj na reakcje!", true);
