...
YCMD:wez(playerid, params[], help)
{
    ...
    if(IsPlayerConnected(playerid))
    {
	    ...
	    if(strcmp(x_job,"drugs",true) == 0 || strcmp(x_job,"dragi",true) == 0)
		{
			...
			if(level >= 0 && level <= 50)
			{ tel = 5; if(ammount < 1 || ammount > 6) { sendTipMessageEx(playerid, COLOR_GREY, "Mo¿esz braæ od 1 do 6g przy tym poziomie Dilera Dragów!"); return 1; } }
			else if(level >= 51 && level <= 100)
			{ tel = 4; if(ammount < 1 || ammount > 12) { sendTipMessageEx(playerid, COLOR_GREY, "Mo¿esz braæ od 1 do 12g przy tym poziomie Dilera Dragów!"); return 1; } }
			else if(level >= 101 && level <= 200)
			{ tel = 3; if(ammount < 1 || ammount > 20) { sendTipMessageEx(playerid, COLOR_GREY, "Mo¿esz braæ od 1 do 20g przy tym poziomie Dilera Dragów!"); return 1; } }
			else if(level >= 201 && level <= 400)
			{ tel = 2; if(ammount < 1 || ammount > 30) { sendTipMessageEx(playerid, COLOR_GREY, "Mo¿esz braæ od 1 do 30g przy tym poziomie Dilera Dragów!"); return 1; } }
			else if(level >= 401)
			{ tel = 1; if(ammount < 1 || ammount > 99) { sendTipMessageEx(playerid, COLOR_GREY, "Mo¿esz braæ od 1 do 99g przy tym poziomie Dilera Dragów!"); return 1; } }
		...
		else if(strcmp(x_job,"fuel",true) == 0 || strcmp(x_job,"kanister",true) == 0 || strcmp(x_job,"paliwo",true) == 0)
		{
		    if(IsAtGasStation(playerid))
			{
			    ...
			    format(string, sizeof(string), "* Wzi¹³eœ kanister z 20L paliwa za $%d",price);
			    SendClientMessage(playerid, COLOR_LIGHTBLUE, string);
		...
		else if(strcmp(x_job,"gasnice",true) == 0 || strcmp(x_job,"gasnica",true) == 0)
		{
			if (CheckPlayerPerm(playerid, PERM_MEDIC))
			{
				...
				if(vehicleid != -1)
    			{
					if(Car_GetOwnerType(vehicleid) == CAR_OWNER_GROUP && (GetVehicleModel(vehicleid) == 407 || GetVehicleModel(vehicleid) == 544))// wszystkie auta frakcji
					{
						...
						{
							sendTipMessage(playerid, "Ten wóz stra¿acki nie nale¿y do LSRC.");
							return 1;
						}
					}
					else
			...
			{
				SendClientMessage(playerid,COLOR_GREY, "Komenda dostêpna tylko dla LSRC.");
				return 1;
			}
		}
