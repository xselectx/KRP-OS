...
YCMD:darkweb(playerid, params[], help)
{
    ...
    if(IsPlayerConnected(playerid))
    {
		...
		else if(PlayerInfo[playerid][pConnectTime] == 0 && PlayerInfo[playerid][pLevel] == 1) return sendErrorMessage(playerid, "Aby pisaæ na darkwebie musisz przegraæ 1h na serwerze!");
		else if(GetPlayerAdminDutyStatus(playerid) == 1) return sendErrorMessage(playerid, "Nie mo¿esz pisaæ na darkwebie podczas s³u¿by administratora!");
        ...
        else if(isnull(params)) return SendClientMessage(playerid, COLOR_GRAD2, "U¯YJ: /darkweb [tekst]");
		...
		{
			sendTipMessage(playerid, "Twój telefon jest wy³¹czony! W³¹cz go za pomoc¹ /p Telefon");
			return 1;
		}
