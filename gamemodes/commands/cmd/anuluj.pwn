...
YCMD:anuluj(playerid, params[], help)
{
	...
	if( sscanf(params, "s[16]", x_job))
	{
		...
		SendClientMessage(playerid, COLOR_GREY, "Dostêpne nazwy: Produkt, Tuning, Tatuaz");
	...
	if(strcmp(x_job,"sex",true) == 0) {	SexOffer[playerid] = 999; SexPrice[playerid] = 0; }
		...
		if(IsPlayerConnected(offer))
			SendClientMessage(offer, COLOR_NEWS, "* Twoja oferta zosta³a odrzucona!");
		SendClientMessage(playerid, -1, "* Odrzuci³eœ oferte sprzeda¿y."); 
	}
