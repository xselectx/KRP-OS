...
YCMD:odpal(playerid, params[], help)
{
    ...
    if(IsPlayerConnected(playerid))
    {
        ...
        if(GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
        {
			...
			if(PlayerInfo[playerid][pInjury] > 0 || PlayerInfo[playerid][pBW] > 0) return sendErrorMessage(playerid, "Jesteœ ranny, nie mo¿esz odpaliæ silnika!");
            if(lHP <= 250.0) return sendErrorMessage(playerid, "Silnik jest zepsuty!"); //        8.12.2016 propozycja uid 30923
			if(handbrake[GetPlayerVehicleID(playerid)]) return sendErrorMessage(playerid, "Nie mo¿esz odpaliæ silnika na rêcznym!");
