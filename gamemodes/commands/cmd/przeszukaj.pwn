			...
			if(giveplayerid != INVALID_PLAYER_ID)
			{
			    if (ProxDetectorS(8.0, playerid, giveplayerid) && Spectate[giveplayerid] == INVALID_PLAYER_ID)
				{
				    if(!IsPlayerInAnyVehicle(playerid) && !IsPlayerInAnyVehicle(giveplayerid))
				    {
					    ...
					    if(PlayerInfo[giveplayerid][pMats] > 0) { format(text2, sizeof(text2), "| Materia³y (%d czêœci)", CountMats(giveplayerid)); } else { text2 = "| Pusta kieszeñ"; }
					    if(PlayerHasFishes(giveplayerid)) { text3 = "| Ryby"; } else { text3 = "| Pusta kieszeñ"; }
					    if(HasItemType(giveplayerid, ITEM_TYPE_PHONEBOOK) != -1) { text4 = "| Ksi¹¿ka tel"; } else { text4 = "| Pusta kieszeñ"; }
					    if(HasItemType(giveplayerid, ITEM_TYPE_MP3) != -1) { text5 = "| MP3"; } else { text5 = "| Pusta kieszeñ"; }
