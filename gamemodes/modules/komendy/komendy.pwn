...
public e_COMMAND_ERRORS:OnPlayerCommandPerformed(playerid, cmdtext[], e_COMMAND_ERRORS:success)
{
	if(success == COMMAND_OK) {
		Log(commandLog, WARNING, "%s wykona³ komendê %s", GetPlayerLogName(playerid), cmdtext);
	}
	return COMMAND_OK;
}
...
public e_COMMAND_ERRORS:OnPlayerCommandReceived(playerid, cmdtext[], e_COMMAND_ERRORS:success)
{
	...
	switch(success)
	{
		...
		{
			sendErrorMessage(playerid, "Ta komenda nie istnieje. Wpisz /pomoc aby zobaczyæ listê dostêpnych komend.");
		}
	}

	if(success == COMMAND_OK) {
		Log(commandLog, WARNING, "%s wpisa³ komendê %s", GetPlayerLogName(playerid), cmdtext);
	}
