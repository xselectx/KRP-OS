...
SprawdzBiznesy(playerid)
{
	for(new i=0; i<MAX_BIZNES; i++)//
    {
            ...
            && GetPlayerVirtualWorld(playerid) == 0)
		{
            ...
            {
                if(PlayerInfo[playerid][pConnectTime] < 2) return sendTipMessageEx(playerid, COLOR_GRAD1, "Tylko gracze z conajmniej przegranymi 2 godzinami mog¹ graæ w kasynie!");
                ...
                SendClientMessage(playerid, COLOR_GREEN, "Kostki - (0.5 proc. podatku) za rzut /kostka || Black Jack - "#PRICE_KASYNO_KARTY"$ za kartê /oczko");
                SendClientMessage(playerid, COLOR_GREEN, "Ko³o fortuny - "#PRICE_KASYNO_KF"$ za obrót /kf");
...
CheckPlayerBusiness(playerid)
{
	...
	if(businessID != INVALID_BIZ_ID)
	{
		if(Business[businessID][b_ownerUID] != PlayerInfo[playerid][pUID])
		{
			...
			Log(errorLog, WARNING, "%s zosta³ odebrany biznes %s z powodu b³êdu w wczytaniu",
