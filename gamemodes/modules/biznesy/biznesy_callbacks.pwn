	...
	if(dialogid == DIALOG_PANEL_BIZ)
	{
		...
		if(response)
		{
			...
			else if(GetPVarInt(playerid, "bizWhatToDo") == 3)
			{
				...
				Log(payLog, WARNING, "%s wp³aci³ do biznesu %s kwotê %d$. Nowy stan: %d$",
			...
			else if(GetPVarInt(playerid, "bizWhatToDo") == 4)
			{
				...
				Log(payLog, WARNING, "%s wyp³aci³ z biznesu %s kwotê %d$. Nowy stan: %d$",
					GetPlayerLogName(playerid),
					GetBusinessLogName(businessID), 
					valueMoney,
					Business[businessID][b_moneyPocket]
	...
	else if(dialogid == D_BIZ_WRITE)
	{
		...
		if(response)
		{
			if(GetPVarInt(playerid, "MustBe") == 0)//Kwota kasy za jaka gracz ma kupic
			{
				...
				ShowPlayerDialogEx(playerid, D_BIZ_WRITE, DIALOG_STYLE_INPUT, "Kotnik Role Play", "WprowadŸ poni¿ej maksymalny zysk\nktóry gracz bêdzie móg³ otrzymaæ co godzine.", "Dalej", "WyjdŸ"); 
				return 1;
			}
			if(GetPVarInt(playerid, "MustBe") == 1)
			{
				...
				ShowPlayerDialogEx(playerid, D_BIZ_WRITE, DIALOG_STYLE_INPUT, "Kotnik Role Play", "Wpisz poni¿ej nazwê biznesu", "Dalej", "WyjdŸ"); 
				return 1;
			}
			if(GetPVarInt(playerid, "MustBe") == 2)
			{
				...
				{
					ShowPlayerDialogEx(playerid, D_BIZ_WRITE, DIALOG_STYLE_INPUT, "Kotnik Role Play", "Wpisz poni¿ej nazwê biznesu\nMINIMALNIE 3 ZNAKI", "Dalej", "WyjdŸ"); 
				}
				return 1;
			}
		}
