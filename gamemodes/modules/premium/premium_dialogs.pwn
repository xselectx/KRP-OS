...
DialogMenuDotacje(playerid)
{
	...
	format(string, sizeof(string), ""#HQ_COLOR_TEKST"Iloœæ KotnikCoins: \t\t"#PREMIUM_EMBED2"%d KC\n" \
...
static DialogKupKP(playerid)
{
	new string[256];
	if(IsPlayerPremium(playerid))
		format(string, sizeof(string), "Mo¿esz przed³u¿yæ konto premium o 30 dni za "INCOLOR_GREEN""#PRZEDLUZ_KP_CENA" Kotnik Coinsów\nCzy chcesz to zrobiæ?");
	else
		format(string, sizeof(string), "Mo¿esz kupiæ konto premium na 30 dni za "INCOLOR_GREEN""#MIESIAC_KP_CENA" Kotnik Coinsów\nCzy chcesz to zrobiæ?");
