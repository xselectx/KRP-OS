...
premium_ConvertToNewSystem(playerid)
{
	if(PlayerInfo[playerid][pDonateRank] != 0)
	{
		...
		_MruGracz(playerid, "Otrzyma³eœ Konto Premium na 3 miesi¹ce i dodatkowe 500 KC do wykorzystania.");

		Log(premiumLog, WARNING, "%s otrzyma³ konwersje KP %d na nowy system", GetPlayerLogName(playerid), PlayerInfo[playerid][pDonateRank]);
...
{
	return _MruGracz(playerid, sprintf("Aktualnie na Twoim koncie znajduje siê %d KotnikCoins.", PremiumInfo[playerid][pMC]));
}
...
DajMC(playerid, mc)
{
	...
	mysql_query(sprintf("UPDATE `mybb_users` SET `samp_kc`=`samp_kc`+'%d' WHERE `uid` = '%d'", mc, PlayerInfo[playerid][pGID]));
...
KupPrzedmiotPremium(playerid, id)
{
	...
	Log(premiumLog, WARNING, "%s kupi³ unikatowy przedmiot %d za %dKC",
	...
	_MruAdmin(playerid, sprintf("Gratulujemy dobrego wyboru. Kupi³eœ przedmiot o ID %d za %d KC.", PrzedmiotyPremium[id][Model], PrzedmiotyPremium[id][Cena]));
...
KupSlotPojazdu(playerid)
{
	...
	Log(premiumLog, WARNING, "%s kupi³ slot wozu za "#CAR_SLOT_CENA"KC",
		GetPlayerLogName(playerid));
	_MruAdmin(playerid, sprintf("Kupi³eœ sobie slot na auto za %d KC. Masz teraz %d slotów.", CAR_SLOT_CENA, MRP_GetPlayerCarSlots(playerid)));
...
KupZmianeNicku(playerid)
{
	...
	Log(premiumLog, WARNING, "%s kupil zmiane nicku za "#ZMIANA_NICKU_CENA"KC",
		GetPlayerLogName(playerid));
	_MruAdmin(playerid, sprintf("Kupi³eœ sobie zmianê nicku za %d KC. Masz teraz %d zmian nicku.", ZMIANA_NICKU_CENA, MRP_GetPlayerNickChanges(playerid)));
...
KupSkinPremium(playerid, id)
{
	...
	Log(premiumLog, WARNING, "%s kupi³ unikatowy skin %d za %dKC",
		GetPlayerLogName(playerid), 
	...
	_MruAdmin(playerid, sprintf("Gratulujemy dobrego wyboru. Kupi³eœ skin o ID %d za %d KC.", SkinyPremium[id][Model], SkinyPremium[id][Cena]));
