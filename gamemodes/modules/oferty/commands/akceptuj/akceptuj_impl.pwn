...
command_akceptuj_Impl(playerid, x_job[32])
{
    ...
    else if(strcmp(x_job, "mandat", true) == 0)
    {
        ...
        if(IsPlayerConnected(policjant))
        {
            if(IsAPolicja(policjant))
            {
                ...
                if(kaska[playerid] < amount) return sendErrorMessage(playerid, "Nie staæ Cie na zaplacenie mandatu");
                ZabierzKaseDone(playerid, amount);
			    DajKaseDone(policjant, amount);
                new str[128];
                format(str, sizeof(str), "Akceptowa³eœ mandat od %s o kwocie $%i!", GetNickEx(policjant), amount);
            ...
            } else sendTipMessage(playerid, "Twój mandat wygas³!");
        } else sendTipMessage(playerid, "Twój mandat wygas³!");
        ...
        if(GuardOffer[playerid] < 999)
        {
            if(kaska[playerid] > GuardPrice[playerid] && GuardPrice[playerid] > 0)
            {
                if(IsPlayerConnected(GuardOffer[playerid]))
                {
                    ...
                    Log(payLog, WARNING, "%s kupi³ kamizelkê od %s za $%d", GetPlayerLogName(playerid), GetPlayerLogName(GuardOffer[playerid]), GuardPrice[playerid]);
                    SendMessageToAdmin(sprintf("%s kupi³ kamizelkê od %s za $%d", GetNickEx(playerid), GetNickEx(GuardOffer[playerid]), GuardPrice[playerid]), COLOR_RED);
            ...
            if(kaska[playerid] > DrugPrice[playerid] && DrugPrice[playerid] > 0)
            {
                if(PlayerInfo[playerid][pDrugs] < 7)
                {
                    if(IsPlayerConnected(DrugOffer[playerid]))
                    {
                        ...
                        Log(payLog, WARNING, "%s kupi³ od %s paczkê %d narkotyków za %d$", GetPlayerLogName(playerid), GetPlayerLogName(DrugOffer[playerid]), DrugGram[playerid], DrugPrice[playerid]);
                        //
                ...
                if(IsPlayerInAnyVehicle(playerid))
                {
                    if(IsPlayerConnected(RepairOffer[playerid]))
                    {
                        if(ProxDetectorS(10.5, playerid, RepairOffer[playerid]))
                        {
                            ...
                            SendClientMessage(playerid, COLOR_LIGHTBLUE, string);
                            format(string, sizeof(string), "* Naprawi³eœ pojazd %s, $%d zostanie dodane do twojej wyp³aty.",sendername,RepairPrice[playerid]);
    ...
    else if(strcmp(x_job,"zaproszenie", true) == 0 || strcmp(x_job,"zapro", true) == 0)
    {
        if(GetPVarInt(playerid, "MotelPendingInviteToRoom") != 0)
        {
            ...
            if(MotelRooms[toRoom][mtrAccessCount] >= MOTEL_MAX_ACCESS) return sendErrorMessage(playerid, "Akceptacja zaproszenia nie powiod³a siê, poniewa¿ pokój jest ju¿ pe³ny!");
            if(strcmp(fromName, GetNick(fromID), true) != 0) return sendErrorMessage(playerid, "Akceptacja zaproszenia nie powiod³a siê, poniewa¿ gracz wyszed³ z gry!");

            Motel_AddUIDToAccess(toRoom, PlayerInfo[playerid][pUID], GetNick(playerid));
            format(str, sizeof(str), "Zaakceptowa³eœ zaproszenie %s i masz teraz dostêp do pokoju.",  nick2);
            sendTipMessage(playerid, str);
            format(str, sizeof(str), "%s zaakceptowa³ twoje zaproszenie i ma teraz dostêp do pokoju.", nick);
        ...
        if(GetPVarInt(playerid, "ZastrzykHave") == 1)
        {
            ...
            if(kaska[playerid] >= 1)
            {
                if(!IsPlayerConnected(giveplayerid))
                {
                    ...
                    sendErrorMessage(playerid, "Gracz który oferowa³ Ci zastrzyk wyszed³ z gry!");
                    return 1;
                }
                ...
                ProxDetector(20.0, playerid, sprintf("* Lekarz %s wyci¹ga strzykawkê i wstrzykuje leki %s.", GetNick(giveplayerid), GetNick(playerid)), 
                COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE
                );
                if(IsPlayerHealthy(playerid)) 
                {
                    SendClientMessage(playerid, COLOR_WHITE, "Lekarz da³ ci zastrzyk i pooprawi³ Twoj¹ odpornoœæ.");
                    ProxDetector(20.0, playerid, sprintf("* %s czuje siê lepiej oraz jego organizm sta³ siê bardziej odporny na choroby.", GetNick(playerid)), 
                        COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE
                    );
                }
                else
                {
                    ...
                    ProxDetector(20.0, playerid, sprintf("* %s czuje siê lepiej oraz jego organizm lepiej radzi sobie z objawami choroby.", GetNick(playerid)), 
                        COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE
                    );
                }
            ...
            {
                SendClientMessage(playerid, COLOR_GREY, "Nie staæ Ciê na zastrzyk. ($1)");
            }
            SetPVarInt(playerid, "ZastrzykID", INVALID_PLAYER_ID);
            SetPVarInt(playerid, "ZastrzykHave", 0);
    ...
    {
        if(PlayerInfo[playerid][pInjury] <= 0) return sendErrorMessage(playerid, "Nie jesteœ ranny!");
        if(!GetPVarInt(playerid, "acceptdeath-allow")) return va_SendClientMessage(playerid, COLOR_GREY, "Musisz odczekaæ jeszcze %d sekund!", (PlayerInfo[playerid][pInjury]));
        ...
        sendTipMessage(playerid, "Twoje bronie oraz poziom poszukiwania zosta³y usuniête!");
    ...
    else if(strcmp(x_job, "tatuaz", true) == 0) //System tatua¿y 4.1
    {
        ...
        if(!IsPlayerNear(playerid, offer)) return sendErrorMessage(playerid, "Nie znajdujesz siê w pobli¿u gracza, który sk³ada³ Ci ofertê!");
        ...
        va_SendClientMessage(offer, COLOR_LIGHTBLUE, "* Gracz %s akceptowa³ twoj¹ ofertê, poczekaj a¿ wybierze wzór.", GetNick(playerid));
