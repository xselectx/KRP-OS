...
graffiti_EditGraffiti(playerid, f)
{
	...
	Log(serverLog, WARNING, "Gracz %s zaaktualizowa³ graffiti. Tekst:%s", GetPlayerLogName(playerid), GraffitiInfo[f][grafText]);
...
graffiti_CreateGraffiti(playerid)
{
	...
	graffiti_ReloadForPlayers(f);
	EditDynamicObject(playerid, GraffitiInfo[f][gID]);
	Log(serverLog, WARNING, "Gracz %s stworzy³ graffiti. Tekst: %s", GetPlayerLogName(playerid), GraffitiInfo[f][grafText]);
	return 1;
}
graffiti_ZerujListe(playerid)
{
